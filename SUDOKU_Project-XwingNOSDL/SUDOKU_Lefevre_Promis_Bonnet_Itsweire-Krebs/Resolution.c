        ...
        for (int y = 0; y < sizet; y++)
        {
            ...
            if (caseVide(&grid[x][y]))
            {
                if (oneNoteLeft(grid[x][y].notes))
                {
                    ...
                    found = true;
                    printf("J'ajoute la valeur - %d - dans la case [%d][%d]\n", getVal(grid,x, y), x, y);
                }
            }
        }
    }
...
//        printf("    ********* Règles 5 et 6 : *********\n");
        result1 = rules_67_zone (grid, X1, Y1, X2, Y2, k, kuplet);      //Ici, pour chaque K-uplet, result1 renvoie "true" si
        if (result1)                                                   //Une note a �t� modifi�e (ie on pourra relancer run_rules depuis le d�but)
        {
            found = true;
        }
//        printf("    ********* Règles 7 et 8 : *********\n");
        result2 = rules_89_zone (grid, X1, Y1, X2, Y2, k, kuplet);     //Pareil que pour result1 mais avec "rules_89_zone"
        if (result2)
        {
            found = true;
        }
...
bool rules_1112_launch(T_grid grid)
{
    ...
    printf("\n----------------- Règles Box-2 et Box-3 réduction ------------------\n");
    ...
    if (testboxes[0][0] == testboxes[1][0])
    {
        for (int i = testboxes[0][0]+1; i < LENGTH; i++)
        {
            if (i != testboxes[0][0] && (caseVide(&grid[i][testboxes[0][1]]) && IsInTheTampon(grid[i][testboxes[0][1]].notes, tmp_testValue)) && (caseVide(&grid[i][testboxes[1][1]]) && IsInTheTampon(grid[i][testboxes[1][1]].notes, tmp_testValue)))
            {
                ...
                {
                    printf("J'ai trouvé un X_wing sur des lignes :\n");
                    ...
                    printf("Note : %d\n"
                           ...
                           "           X \n"
                           "          / \\ \n"
                           "    [%d][%d]  [%d][%d]\n", getvalNote(tmp_testValue), foundboxes[0][0], foundboxes[0][1], foundboxes[1][0], foundboxes[1][1], foundboxes[2][0], foundboxes[2][1], foundboxes[3][0], foundboxes[3][1]);
                    return true;
                }
            }
        }
    ...
    else
    {
        for (int i = testboxes[0][1]+1; i < LENGTH; i++)
        {
            if (i != testboxes[0][1] && (caseVide(&grid[testboxes[0][0]][i]) && IsInTheTampon(grid[testboxes[0][0]][i].notes, tmp_testValue)) && (caseVide(&grid[testboxes[1][0]][i]) && IsInTheTampon(grid[testboxes[1][0]][i].notes, tmp_testValue)))
            {
                ...
                {
                    printf("J'ai trouvé un X_wing sur des colonnes :\n");
                    foundboxes[0][0] = testboxes[0][0];
                    foundboxes[0][1] = testboxes[0][1];
                    foundboxes[1][0] = testboxes[1][0];
                    foundboxes[1][1] = testboxes[1][1];
...
//                for(int i = 0; i<LENGTH; i++)
//                {
//                    add_Value(grid, x, y, i);
//                    backtracking(grid);
//                    deleteAvalue(grid, x, y);
//                }
                int tmp = grid[x][y].notes;
                //Gestion des valeurs dans la note
                ...
                {
                    nbiter += 1;
                    grid = add_Value(grid, x, y, tab[count]);
                    count++;
                    if (verifbetweenrules(grid) && backtracking2(grid))
                    {
                        found = true;
                        break;
                    }
