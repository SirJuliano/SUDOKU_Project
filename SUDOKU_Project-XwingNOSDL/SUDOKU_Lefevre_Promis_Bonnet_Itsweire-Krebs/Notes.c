...
//            printf("Modification des notes sur la ligne : %d\n", i);
//            printf("    Je modifie la note %d, dans la case [%d][%d]\n", numNote, i, y);
        }
    }
}
...
//            printf("Modification des notes sur la colonne : %d\n", i);
//            printf("    Je modifie la note %d, dans la case [%d][%d]\n", numNote, x, i);
        }
    }
}
...
//                printf("Modification des notes sur le bloc de coordonnées [%d][%d] de taille %d\n", xs, ys, nbr);
//                printf("    Je modifie la note %d, dans la case [%d][%d]\n", numNote, X, Y);
            }
        }
    }
    ...
    for(int X = x1_zone; X <= x2_zone ; X++)
    {
        for(int Y = y1_zone; Y <= y2_zone; Y++)           //On parcours la zone travaillée
        {
            ...
            if(opere && getVal(grid, X, Y) == 0)
            {
                ...
                //On applique le calcul supprimant les notes voulues
            ...
            if(opere && found){
                if(compteurAffichage <1){
                    printf("J'enlève la/les note(s) ");
                    afficheNote(tmp);
                    printf("\n");
                    compteurAffichage += 1;
                }

                printf("        de la case [%d][%d]\n",X, Y);
            }
        }
    }
