...
{
    T_grid tmp = createGrid(LENGTH);
    initGrid(tmp, LENGTH);
    for(int i = 0 ; i<LENGTH; i++){
        for(int y = 0; y<LENGTH; y++){
            if(i != pos_line || y != pos_column){
                if(getVal(grid, i, y) != 0){
                    tmp = add_Value(tmp, i, y, getVal(grid, i, y));
                }
            }
        }
    }
    ...
    for(int i = 0 ; i<LENGTH; i++){
        for(int y = 0; y<LENGTH; y++){
            if(i != pos_line || y != pos_column){
                if(getVal(tmp, i, y) != 0){
                    grid = add_Value(grid, i, y, getVal(tmp, i, y));
                }
            }
        }
    }
...
//renvoie le chiffre de la case x, y
int getVal(T_grid grid, int x, int y)
{
    return grid[x][y].value;
}

int getNote(T_grid grid, int x, int y)
{
    return grid[x][y].notes;
}
