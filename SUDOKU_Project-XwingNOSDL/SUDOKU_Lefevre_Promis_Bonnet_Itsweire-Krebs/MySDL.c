        ...
        while (SDL_PollEvent(&event) != 0) {
            if (event.type == SDL_QUIT) {
                ...
                if(pointInsideRect(mouseX, mouseY, &grille)){ //vérifie que leclick de souris était sur une case de la grille
                    ...
                    if(x != -1 && y != -1){
                        if(getNote(grid,y-1, x-1) ==0) {
                            printf("** J'enlève la valeur - %d - dans la case [%d][%d] **\n", getVal(grid, y-1, x-1),y-1, x-1);
                            grid = deleteAvalue(grid, y-1, x-1);
                        } else {
                            printf("Il n'y a pas de valeur dans la case selectionnée\n");
                        }
                    } else {
                        printf("Aucune valeur selectionnée, aucune suppression \n");
                    }
                } else if (pointInsideRect(mouseX, mouseY, &tab[8].rect)){ // bouton règle x_wing
                    bool test = backtracking2(grid);
                    printnbiter();
