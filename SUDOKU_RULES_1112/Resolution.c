...
bool gridVerif(T_grid grid, int X1, int X2, int Y1, int Y2, int length)
{
...
{
    return gridVerif(grid, x, (x + NBSQRT), y, (y + NBSQRT), length);
}
...
bool verifGrid(T_grid grid, int length)
{
    ...
    for(int i = 0; i < length; i++)
    {
        if(!verifLine(grid, i, length) || !verifColumn(grid, i, length))
        {
            return false;
        }
    }

    for (int j = 0; j < NBSQRT; j++)          //parcours des carr�s de chaque lignes
    {
        for (int k = 0; k < NBSQRT; k++)      //parcours des carr�s de chaque colonnes
        {
            if (!verifSquare(grid, (j * NBSQRT), (k * NBSQRT), length))     //on donne les coordonn�es des cases situ�es en haut � gauche des carr�s
            {
                return false;
            }
        }
