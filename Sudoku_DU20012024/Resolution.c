...
{
    printf("JE RENTRE DANS AVAILABLEVALUES\n");
    bool btab[LENGTH];
    for(int i = 0; i<LENGTH;i++)
    {
        btab[i]=true;
    }
    printf("J'ENTAME LA PREMIERE BOUCLE \n");
    printf("X1 = %d   et    X2 = %d    et       Y1 = %d    et Y2 = %d\n", X1,X2,Y1,Y2);
    for(int X = X1; X <=X2; X++)
    {
        for(int Y = Y1; Y <= Y2; Y++)
        {
            printf("ALLER LES BLEUS\n");
            if(grid[X][Y].value != 0)
            {
    ...
    printf("J'AI PASSE LA PREMIERE BOUCLE \n");
